...
namespace JeffLib
{
	public static class CommonGraphics
	{
		...
		{
			DrawCircle(g, center_x, center_y, r, num_segments, true, (float)Math.PI * 2f);
		}

		public static void DrawCircle(Graphics g, float center_x, float center_y, float r, int num_segments, bool fill)
		{
			DrawCircle(g, center_x, center_y, r, num_segments, fill, (float)Math.PI * 2f);
		}
		...
		public static void DrawCircle(Graphics g, float center_x, float center_y, float r, int num_segments, bool fill, float max_angle)
		{
			...
			{
				float num3 = r * (float)Math.Cos(num2);
				float num4 = r * (float)Math.Sin(num2);
			...
			{
				DrawPoly(g, array, num_segments);
			}
		}

		public static void DrawPoly(Graphics g, Point[] vertices, int num_vertices)
		{
			for (int i = 0; i < num_vertices; i++)
			{
				Point point = vertices[(i != num_vertices - 1) ? (i + 1) : 0];
